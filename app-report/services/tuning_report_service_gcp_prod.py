...
def generate_tuning_report_with_agent(request: TuningReport) -> TuningReportResponse:
    ...
    try:
        ...
        # 노드 추가
        workflow.add_node("connection_finder_node", connection_finder_node)
        workflow.add_node("topic_planner_node", topic_planner_node)
        workflow.add_node("researcher_node", researcher_node)
        workflow.add_node("creative_concept_node", creative_concept_node)
        workflow.add_node("content_generator_node", content_generator_node)
        workflow.add_node("editor_node", editor_node)

        # 엣지 연결 (순차적 흐름)
        workflow.set_entry_point("connection_finder_node")
        workflow.add_edge("connection_finder_node", "topic_planner_node")
        workflow.add_edge("topic_planner_node", "researcher_node")
        workflow.add_edge("researcher_node", "creative_concept_node")
        workflow.add_edge("creative_concept_node", "content_generator_node")
        workflow.add_edge("content_generator_node", "editor_node")
        workflow.set_finish_point("editor_node")
